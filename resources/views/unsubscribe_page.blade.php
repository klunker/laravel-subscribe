...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} | Unsubscribe</title>
...
<div
    ...
    @if(! $subscriber )
        <div
            class="max-w-[896px] w-full text-[32px] dark:text-[#fefefe] text-[#1b1b18] font-medium text-start">
            <h1 class="flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor"
                     class="me-3 text-[#f53003] dark:text-[#FF4433]" viewBox="0 0 16 16">
                    <path
            ...
            <p class="text-[16px] leading-[22px]">
                Something went wrong. This unsubscribe link is invalid or expired..
            </p>
            <p class="muted mt-2 text-[16px]">
                Go back to <a href="/" class="text-[#f53003] dark:text-[#FF4433]">home</a>.
            </p>
        </div>
    @else
        <div
            ...
            Unsubscribe
            <p class="text-[16px] leading-[22px]">
                Thank you for using our service. We're sorry to see you go.<br/>
                Would you like to unsubscribe from all emails or just adjust your preferences?
            </p>
        </div>
        <main class="flex flex-col max-w-[335px] w-full lg:max-w-4xl mt-3">
            <div
                class="text-[13px] leading-[20px] flex-1 p-3 pb-12 lg:p-10 bg-white dark:bg-[#161615] dark:text-[#EDEDEC] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg">
                <h1 class="mb-1 font-medium">Select your preferences</h1>
                <form>
                    @method('PATCH')
                    <input type="hidden" name="token" value="{{ request()->get('token') }}">
                    <details class="my-4 bg-[#f5f5f5] dark:bg-[#161615] rounded-lg p-4">
                        <summary
                            class="cursor-pointer text-base font-medium leading-normal text-[#1b1b18] dark:text-[#e3e3e0]">
                            Adjust my preferences
                            <p class="text-[14px] leading-[18px] ms-[18px]">
                                ...
                                @foreach(\Klunker\LaravelSubscribe\Facades\Subscribe::getChannels() as $key => $channel)
                                    <li>
                                        <label class="flex flex-col items-start gap-2">
                                            <p class="text-[14px] leading-[22px] font-bold">
                                                <input type="checkbox" name="subscribe_on[]"
                                                       value="{{ $key }}"
                                                       class="form-checkbox me-2">
                                                {{ ucfirst($key) }}<br/><span
                                                    class="text-[13px] font-normal ms-[25px]">{{ $channel }}</span>
                                            </p>
                                        </label>
                                    </li>
                                @endforeach
                    ...
                    <ul class="flex flex-col my-4  bg-[#f5f5f5] dark:bg-[#161615] rounded-lg p-4 gap-3">
                        <li class="bg-[#f5f5f5] dark:bg-[#353535] rounded-lg p-4">
                            <label class="flex flex-col items-start gap-0">
                                ...
                                <p class="text-[14px] leading-[18px] ms-[25px]">
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            let action_description = 'updated';
            let action = '{{ route('api.subscribe.update') }}';

            const el = document.querySelector('[name="subscribe_all"]');
            if (el.checked) {
                document.querySelector('[name="_method"]').value = 'DELETE';
                action_description = 'deleted';
                action = '{{ route('api.subscribe.delete_by_token') }}';
            ...
            fetch(action, {
                ...
                .then(data => {
                    if (data.result) {
                        ...
                        h2.textContent = 'Subscribe on newsletters was ' + action_description;
                        h2.className = 'text-[32px] dark:text-[#fefefe] text-[#1b1b18] font-medium text-start';


                        const p = document.createElement('p');
                        p.textContent = 'Go back to ';
                        p.className = 'mt-2 mb-0 text-[16px] leading-[22px]';
                        const link = document.createElement('a');
                        link.href = '/';
                        link.textContent = 'home';
                        link.className = 'text-[#f53003] dark:text-[#FF4433]';
                        p.appendChild(link);
                        document.querySelector('main > div').replaceWith(h2);
